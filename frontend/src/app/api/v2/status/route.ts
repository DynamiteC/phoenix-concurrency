...
import {insightQuery, insightSql, INSIGHT_DATABASE} from '@/lib/insights'
...
export async function GET(): Promise<NextResponse<InsightStatusResponse | ApiError>> {
  const t0 = Date.now()
  try {
    const result = await insightQuery(insightSql('insight_status.sql'), {frozen_before: FROZEN_BEFORE})
