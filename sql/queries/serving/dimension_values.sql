...
-- never joins.
SELECT 'platform'    AS dim, platform    AS value, platform    AS label FROM concurrency_deltas WHERE minute < {frozen_before:String} GROUP BY 1, 2, 3
UNION ALL
SELECT 'country'     AS dim, country     AS value, country     AS label FROM concurrency_deltas WHERE minute < {frozen_before:String} GROUP BY 1, 2, 3
UNION ALL
SELECT 'video_type'  AS dim, video_type  AS value, video_type  AS label FROM concurrency_deltas WHERE minute < {frozen_before:String} GROUP BY 1, 2, 3
UNION ALL
SELECT 'app_version' AS dim, app_version AS value, app_version AS label FROM concurrency_deltas WHERE minute < {frozen_before:String} GROUP BY 1, 2, 3
UNION ALL
