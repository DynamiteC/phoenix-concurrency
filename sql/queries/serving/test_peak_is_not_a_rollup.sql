...
(
    ...
    (
        ...
        FROM (SELECT platform, minute, sum(delta) AS d FROM concurrency_deltas
              WHERE minute < {frozen_before:String} GROUP BY platform, minute)
    )
    GROUP BY platform
),
overall AS
(
    ...
    (
        ...
        FROM (SELECT minute, sum(delta) AS d FROM concurrency_deltas
              WHERE minute < {frozen_before:String} GROUP BY minute)
    )
),
live_only AS
(
    ...
    (
        SELECT minute, toInt64(sum(d) OVER (ORDER BY minute ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)) AS c
        FROM (SELECT minute, sum(delta) AS d FROM concurrency_deltas
              WHERE minute < {frozen_before:String} AND video_type = 'live' GROUP BY minute)
    )
),
one_platform AS
(
    ...
    (
        SELECT minute, toInt64(sum(d) OVER (ORDER BY minute ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)) AS c
        FROM (SELECT minute, sum(delta) AS d FROM concurrency_deltas
              WHERE minute < {frozen_before:String} AND platform = 'SONY_ANDROID_TV' GROUP BY minute)
    )
)
SELECT * FROM
(
