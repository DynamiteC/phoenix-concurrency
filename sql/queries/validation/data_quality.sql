...
SELECT * FROM
(
    SELECT 'events total' AS check, toInt64(count()) AS n, 'baseline' AS rule FROM raw_events
    UNION ALL
    ...
    FROM raw_events WHERE content_id <= 0
    ...
    FROM raw_events WHERE content_id NOT IN (SELECT content_id FROM content)
    UNION ALL
    ...
    FROM raw_events WHERE session_start_epoch > event_timestamp
    UNION ALL
    SELECT 'events with a non-positive timestamp', toInt64(count()), 'excluded from derivation'
    FROM raw_events WHERE toUnixTimestamp(event_timestamp) <= 0
    UNION ALL
    ...
    FROM (SELECT video_session_id, toDateTime(event_timestamp) AS ts FROM raw_events
    ...
    FROM (SELECT video_session_id FROM raw_events WHERE event_type = 'VideoSessionStart'
    ...
    FROM (SELECT video_session_id FROM raw_events WHERE event_type = 'VideoSessionEnd'
          GROUP BY 1 HAVING count() > 1)
    UNION ALL
    ...
    FROM (SELECT video_session_id FROM raw_events
    ...
    FROM (SELECT video_session_id FROM raw_events GROUP BY 1 HAVING uniqExact(platform) > 1)
    UNION ALL
    SELECT 'sessions reporting more than one user_id', toInt64(count()),
           'first-seen value wins; affects user-level concurrency only'
    FROM (SELECT video_session_id FROM raw_events GROUP BY 1 HAVING uniqExact(user_id) > 1)
    UNION ALL
    ...
    FROM (SELECT video_session_id FROM raw_events GROUP BY 1 HAVING uniqExact(content_id) > 1)
    ...
    FROM (SELECT (toUnixTimestamp(event_timestamp) - lagInFrame(toUnixTimestamp(event_timestamp))
          ...
          FROM raw_events) WHERE gap > 3600
    ...
    FROM session_minute_runs WHERE run_end < run_start
)
ORDER BY check;
